...
const DevelopMentSteps = () => {
  ...
  return (
    <section className="my-5 mx-auto items-center 2xl:pl-60 xl:pl-40 lg:pl-20 md:pl-10 2xl:pr-60 xl:pr-40 lg:pr-20 md:pr-10 sm:px-10 px-5 lg:gap-6 sm:gap-5 xxs:gap-3 text-center">
      <div className="relative lg:h-[600px] md:h-[600px] sm:h-[550px]  xsm:h-[640px] xxs:h-[700px] xxmm:h-[650px] flex flex-col items-center ">
        <div className="flex flex-col items-center gap-y-10 lg:gap-y-[5rem] md:gap-y-[6rem] sm:gap-y-[5rem] xs:gap-y-[5rem] xsm:gap-y-[5rem] xxmm:gap-y-[5rem] xxs:gap-y-[6rem] absolute  z-40">
          <div className=" flex flex-col gap-y-1 text-left">
            <h1
              className="text-left bg-white 
            xxmm:max-w-[82%] xs:max-w-[95%] w-fit pr-2 inline xl:text-[2.2rem] xxs:leading-7 md:leading-[66px]  sm:leading-8 font-[800]  lg:text-[35px] md:text-[35px] xsm:text-[20px]  xxs:text-[20px] "
            >
          ...
          <div className="grid grid-cols-2 xsm:grid-cols-2 sm:grid-cols-3 xxs:px-4 xxs:gap-y-5 xxs:gap-x-6  xs:gap-y-6 sm:gap-y-8 gap-x-4 xxs:w-[95%] xsm:w-[80%] mx-auto z-40">
            {divContent.map((item, index) => (
              ...
              >
                ...
                />
                <h3 className="md:text-[1.1rem]  lg:text-[1.4rem] xxs:text-[0.8rem]  sm:text-[1rem] text-[#0068FF] font-semibold leading-16 text-center ">
                  {item.title}
                </h3>
              </div>
            ))}
        ...
        <div className=" xxmm:w-[95%] xsm:h-[93%] xxmm:h-[81%] xxs:w-full xxs:h-full  flex flex-col items-center justify-start z-0">
          {/* BAR----1\ */}
          <div
            className="w-full lg:h-[10rem] md:h-[12rem] sm:h-[12rem]  xs:h-[11rem] xsm:h-[14rem] xxmm:h-[17rem] xxs:h-[16rem]
            md:mt-[2.1rem] sm:mt-[1.2rem]  xsm:mt-[0.7rem] mt-1 border-2  border-[#0064B0] border-dashed border-l-0 border-b-0 "
          ></div>
          {/* BAR---2\ */}

          <div className="w-full lg:h-[7rem] md:h-[6rem] sm:h-[5rem] xs:h-[5rem] xsm:h-[5rem] xxs:h-[5rem] xxmm:h-[6rem] border-2  border-[#0064B0] border-dashed border-r-0 "></div>
          {/* BAR---3\ */}

          <div className="w-full lg:h-[10rem] md:h-[9rem] sm:h-[7.6rem] xs:h-[6rem] xsm:h-[6rem] xxmm:h-[7rem] xxs:h-[6.5rem] relative">
          ...
          <div className="w-full relative sm:h-0 sm:w-0   xs:h-[6.5rem] xsm:h-[6rem] xxmm:h-[8rem] xxs:h-[6rem] border-2  border-[#0064B0] border-dashed border-t-0 border-r-0 ">
